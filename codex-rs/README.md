...
When all remaining open work is blocked on the same long-running job, wait, or quiet active turn, the runtime enters `waiting(long-task)`. A wait only counts as a long-task blocker when it is trackable or explicitly long-running, such as an `AUTO_CHECK` wait, external API/MCP/queue wait, training run, benchmark, Docker build, download, compile, or similar asynchronous dependency. Ordinary final artifact or audit gates do not qualify; those should keep the owner/lead runnable. In `waiting(long-task)` the runtime suppresses token-spending automatic prompts such as lead ticks, task watchdog nudges, idle wakeups, heartbeats, idle outreach, AI digest turns, node asset sync, and dynamic member sync. It keeps only completion detection active: app-server event drain, job refresh, `AUTO_CHECK` polling, explicit user messages, and real completion/failure evidence. Non-user mailbox chatter is marked with `team_wait_idle_mailbox_suppressed`; explicit user input is left pending as `team_wait_idle_user_message_pending` and is allowed to wake normal steering. Once the blocker clears, normal orchestration resumes.
